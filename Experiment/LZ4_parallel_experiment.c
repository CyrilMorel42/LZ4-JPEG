...
int main() {
    ...
    FILE *fp = fopen("results/LZ4_par.exe_execution_times.json", "w");
    int sizes[] = {350};
    ...
    for (size_t i = 0; i < (sizeof(sizes)/sizeof(int)); i++) {
        ...
        fprintf(fp, "    {\n");
        fprintf(fp, "        \"text\": %d,\n", text_size);
        fprintf(fp, "        \"exe_name\": \"LZ4_par.exe\",\n");
        ...
        for (size_t j = 0; j < runs; j++) {
            ...
            while (result != 0) {
                ...
                FILE *pipe = popen("LZ4_par.exe 2>&1", "r");
                if (!pipe) {
                    printf("[ERROR] Failed to execute LZ4_par.exe\n");
                    return 1;
                }
