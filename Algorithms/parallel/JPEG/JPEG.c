...
void inverse_discrete_cosine_transform(uint8_t *values, size_t width, size_t height, double *coefficients)
{
    ...
    for (size_t i = 0; i < width * height; i++)
    {
        ...
        values[i] = (value < 0) ? 0 : (value > 255) ? 255
                                                    : (uint8_t)value;
    }
...
PixelGroup *divide_image(uint8_t **luminance_values, uint8_t **rChrominance_values, uint8_t **bChrominance_values, ImageData data, size_t group_size)
{
    ...
    for (size_t row = 0; row < data.height; row++)
    {
        for (size_t col = 0; col < data.width; col++)
        {
            ...
            if (local_col % 2 == 0)
            { // Process chrominance for even columns
...
{
    // printf("[DEBUG] Entering encode_huffman()...\n");

    if (input == NULL || input_len == 0)
    {
        printf("[ERROR] Invalid input to encode_huffman()!\n");
        return NULL;
    }
    ...
    if (!frequencies)
    {
        printf("[ERROR] Memory allocation failed for frequencies.\n");
        return NULL;
    }

    size_t unique_count = 0;
    calculate_frequency(frequencies, input_len, input, &unique_count);
    // printf("[DEBUG] Unique symbols count: %zu\n", unique_count);

    // Allocate memory for heap
    Node *heap = malloc(sizeof(Node) * unique_count);
    if (!heap)
    {
    ...
    *root_out = root; // Store root for decoding

    // Allocate memory for Huffman codes
    HuffmanCode *codes = malloc(sizeof(HuffmanCode) * unique_count);
    if (!codes)
    {
        printf("[ERROR] Memory allocation failed for Huffman codes.\n");
        free(frequencies);
        free(heap);
        return NULL;
    }
    ...
    *code_count = code_index;
    ...
    // printf("[DEBUG] Huffman encoding complete. Codes allocated at: %p\n", (void*)codes);

    // Debug: Print Huffman codes
    // for (size_t i = 0; i < *code_count; i++) {
    //     printf("[DEBUG] Code %zu: Value = %d, Code = %s\n", i, codes[i].value, codes[i].code);
    // }

    // Free frequency table (not needed anymore)
    free(frequencies);
...
{
    parallel_args *args = (parallel_args *)lpParam;
    PixelGroup *block = &(args->block);

    discrete_cosine_transform(block->lum_values, 8, 8, &(block->lum_coefficients));
    discrete_cosine_transform(block->r_values, 4, 8, &(block->r_coefficients));
    discrete_cosine_transform(block->b_values, 4, 8, &(block->b_coefficients));

    Quantize(&(block->lum_coefficients), LUMINANCE_QUANTIZATION_TABLE, 64);
    Quantize(&(block->b_coefficients), CHROMINANCE_QUANTIZATION_TABLE, 32);
    Quantize(&(block->r_coefficients), CHROMINANCE_QUANTIZATION_TABLE, 32);

    double transformed_lum[64];
    double transformed_r[32];
    double transformed_b[32]; // temporary array for zigzagged coefficients
    zigzag_pattern(8, 8, block->lum_coefficients, transformed_lum);
    zigzag_pattern(4, 8, block->r_coefficients, transformed_r);
    zigzag_pattern(4, 8, block->b_coefficients, transformed_b);

    // Copy the transformed data back to the block's lum_coefficients
    for (size_t j = 0; j < 64; j++)
    {
        block->lum_coefficients[j] = transformed_lum[j];
    }
    for (size_t j = 0; j < 32; j++)
    {
        block->r_coefficients[j] = transformed_r[j];
        block->b_coefficients[j] = transformed_b[j];
    }
    ...
    RLE(transformed_lum, 64, &(block->RLE_encoded_lum), &encoded_length_lum);
    RLE(transformed_r, 32, &(block->RLE_encoded_r), &encoded_length_r);
    RLE(transformed_b, 32, &(block->RLE_encoded_b), &encoded_length_b);

    //     // Encode and decode luminance (64 elements)
    size_t code_count_lum;
    Node *root_lum;
    HuffmanCode *codes_lum = encode_huffman(block->RLE_encoded_lum, encoded_length_lum, &code_count_lum, &root_lum);

    char encoded_sequence_lum[1024];
    generate_encoded_sequence(block->RLE_encoded_lum, encoded_length_lum, codes_lum, code_count_lum, encoded_sequence_lum);
    ...
    {
        block->RLE_encoded_lum[j] = (int)decoded_output_lum[j];
    }

    if (codes_lum == NULL)
    {
        printf("[ERROR] Attempting to free NULL pointer (codes_lum)!\n");
    }
    else
    {
        // printf("[DEBUG] Freeing Huffman codes at: %p\n", (void*)codes_lum);
        free(codes_lum);
    }
    ...
    size_t code_count_r;
    Node *root_r;
    HuffmanCode *codes_r = encode_huffman(block->RLE_encoded_r, encoded_length_r, &code_count_r, &root_r);

    char encoded_sequence_r[512];
    generate_encoded_sequence(block->RLE_encoded_r, encoded_length_r, codes_r, code_count_r, encoded_sequence_r);
    ...
    {
        block->RLE_encoded_r[j] = (int)decoded_output_r[j];
    }
    ...
    size_t code_count_b;
    Node *root_b;
    HuffmanCode *codes_b = encode_huffman(block->RLE_encoded_b, encoded_length_b, &code_count_b, &root_b);

    char encoded_sequence_b[512];
    generate_encoded_sequence(block->RLE_encoded_b, encoded_length_b, codes_b, code_count_b, encoded_sequence_b);
    ...
    {
        block->RLE_encoded_b[j] = (int)decoded_output_b[j];
    }
    ...
    inverse_RLE(block->RLE_encoded_lum, block->lum_coefficients, 64, encoded_length_lum);
    inverse_RLE(block->RLE_encoded_r, block->r_coefficients, 32, encoded_length_r);
    inverse_RLE(block->RLE_encoded_b, block->b_coefficients, 32, encoded_length_b);
    ...
    reverse_zigzag_pattern(8, 8, block->lum_coefficients, reverse_transformed_lum);
    reverse_zigzag_pattern(4, 8, block->r_coefficients, reverse_transformed_r);
    reverse_zigzag_pattern(4, 8, block->b_coefficients, reverse_transformed_b);

    // Copy the reverse-transformed data back to the block's lum_coefficients
    for (size_t j = 0; j < 64; j++)
    {
        block->lum_coefficients[j] = reverse_transformed_lum[j];
    }
    for (size_t j = 0; j < 32; j++)
    {
        block->r_coefficients[j] = reverse_transformed_r[j];
        block->b_coefficients[j] = reverse_transformed_b[j];
    }

    Inverse_quantize(&(block->lum_coefficients), LUMINANCE_QUANTIZATION_TABLE, 64);
    Inverse_quantize(&(block->b_coefficients), CHROMINANCE_QUANTIZATION_TABLE, 32);
    Inverse_quantize(&(block->r_coefficients), CHROMINANCE_QUANTIZATION_TABLE, 32);

    inverse_discrete_cosine_transform(block->lum_values, 8, 8, block->lum_coefficients);
    inverse_discrete_cosine_transform(block->b_values, 4, 8, block->b_coefficients);
    inverse_discrete_cosine_transform(block->r_values, 4, 8, block->r_coefficients);
}
...
int main()
{
    ...
    HANDLE *threads = malloc(total_blocks * sizeof(HANDLE));


    DWORD *threads_IDs = malloc(total_blocks * sizeof(DWORD));


    for (size_t i = 0; i < total_blocks; i++)
    {
        parallel_args *args = malloc(sizeof(parallel_args));
        args->block = blocks[i];
        threads[i] = CreateThread(NULL, 0, process, args, 0, &threads_IDs[i]);
    }

    WaitForMultipleObjects(total_blocks, threads, TRUE, INFINITE);
