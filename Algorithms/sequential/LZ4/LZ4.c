...
#define WINDOW_SIZE 65535        // limited by the offset length (2 bytes)
...
//helper function to print the hexadecimal content of a file
...
void ensure_directories()
{
    ...
    { // an input file is necessary, so fatal error if inexistant
...
//function to write a sequence output to a file
void write_sequence(LZ4Sequence sequence, FILE *file)
{
    ...
    //handles case where bytes must be added to the sequence
    ...
    //handles the case where bytes must be added to the sequence
    if (sequence.match_length >= 4)
    {
        uint8_t adjusted_match_length = sequence.match_length - 4;

        if (adjusted_match_length >= 15)
        { // handles the case where the match length excedes what can be written in the token, if match_length = 15, the 0 in next byte needs to be written anyway
            ...
            fwrite(&remaining, sizeof(uint8_t), 1, file); // writes the last incomplete byte of the overflow or the only incomplete byte
        }
    }
}
...
void block_encode(const char *block_entry, size_t block_length, LZ4Block *block, FILE *log_file, FILE *output_file, LZ4Frame *frame)
{
    ...
    while (input_index < block_length)
    {
        ...
        { // writes the match
            ...
            uint8_t token_match_length = (match_length >= 19) ? 15 : match_length - MIN_MATCH_LENGTH; // actual match length is max 19 in the token, gain of 4 bits because of the lower limit
            ...
            { // handles the case where the literal length excedes 15 and adjust the byte_size accordingly
            ...
            if (adjusted_match_length >= 15)
            { // handles the case where the match length excedes 15 and adjust the byte_size accordingly
                uint8_t remaining = adjusted_match_length - 15;
    ...
    if (literal_counter > 0)
    { // handles the case where there is not match at the end of a block, and encodes all the literals with match length 0
...
void interpret_sequence(LZ4Sequence sequence, uint8_t *decompressed_buffer, size_t *decompressed_size)
{
    ...
    if (sequence.match_offset != 0)
    {
        ...
        for (size_t i = 0; i < sequence.match_length; i++)
        {
            ...
            // printf("%c", decompressed_buffer[match_pos]);
